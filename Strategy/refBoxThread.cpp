...
namespace Strategy
{
  bool RefBoxThread::instantiated = false;
  ...
  RefBoxThread::RefBoxThread(RefBoxCmd* refBoxCmdSh,
    ...
    refBoxCS(refBoxCS)
  {
    assert(instantiated == false); // Preventing this class from being instantiated multiple times
    instantiated = true;
  ...
  void RefBoxThread::run()
  {
    while (true)
    {
      try
      {
        sock->recvFrom(refBoxCmd, sizeof(RefBoxCmd), REF_BOX_MULTICAST_ADDR, REF_BOX_MULTICAST_PORT);
